exports.handler = async (event) => {
  ...
  if (!to || !protocol) {
    return { statusCode: 400, headers, body: JSON.stringify({ error: "Campos obrigatorios: to, protocol" }) };
  }
  ...
  if (emailProvider === 'resend') {
    ...
    if (!apiKey) return { statusCode: 500, headers, body: JSON.stringify({ error: "RESEND_API_KEY nao configurado" }) };
    ...
    if (!resp.ok) return { statusCode: 500, headers, body: JSON.stringify({ error: `Resend: ${JSON.stringify(result)}` }) };
    return { statusCode: 200, headers, body: JSON.stringify({ success: true, provider: 'resend', id: result.id }) };
  }

  // Default: log mode (no email sent, just confirms receipt)
  console.log(`[Email] Would send to ${to}: Pedido ${protocol} — ${schoolName} — R$ ${(total||0).toFixed(2)}`);
  return { statusCode: 200, headers, body: JSON.stringify({ success: true, provider: 'log', message: 'Email registrado (modo log). Configure EMAIL_PROVIDER=resend para envio real.' }) };
};
