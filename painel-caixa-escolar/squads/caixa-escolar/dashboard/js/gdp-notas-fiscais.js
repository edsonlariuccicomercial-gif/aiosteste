...
// ===== CURA DEFINITIVA AUTOMÁTICA (2026-06-26) — status segue a PROVA, sem nada manual =====
// CAUSA RAIZ (confirmada pela DANFE da NF 1621: chave 3126...6086 + protocolo 131267669874406, mas
// status 'pendente'): a nota autorizou de verdade e CAPTUROU a prova (chave 44 díg + protocolo), porém
// nf.status ficou dessincronizado ('rascunho_nf_real'/'transmissao_realizada') por um eco/reload. O
// auto-fix antigo (gdp-init.js) só corrigia por integracoes.sefaz.cStat==='100' — campo VOLÁTIL que o
// eco zera. Esta função corrige pela PROVA DURÁVEL (temProvaAutorizacao = chave+protocolo), que está no
// próprio objeto: se há prova mas o status não é 'autorizada', sincroniza AUTOMATICAMENTE. Sem botão,
// sem throttle, sem consultar a SEFAZ (a prova já é local). Idempotente. Roda no boot e em toda
// reconciliação. REGRA: NUNCA cria prova — só promove status quando a prova JÁ existe localmente.
function autoCurarStatusPorProva() {
  var curadas = 0;
  try {
    (notasFiscais || []).forEach(function (nf) {
      if (!nf || !isNotaFiscalReal(nf)) return;
      if (nf.status === "autorizada") return;
      if (!temProvaAutorizacao(nf)) return; // só promove com prova durável (chave 44 díg + protocolo)
      nf.status = "autorizada";
      nf.sefaz = nf.sefaz || {};
      ...
      if (typeof setIntegrationState === "function") {
        setIntegrationState(nf, "sefaz", {
          status: "autorizada",
          lastAction: "autocura_status_por_prova",
          accessKey: String(nf.sefaz.chaveAcesso || nf.chaveAcesso || ""),
          protocol: String(nf.sefaz.protocolo || nf.protocolo || "")
        });
      }
      try { saveNotasFiscais(nf.id); } catch (_) {}
      try { if (window.gdpApi && window.gdpApi.notas_fiscais) window.gdpApi.notas_fiscais.save(nf); } catch (_) {}
      // cobrança faltante idempotente (1 título por nota) — a nota autorizada deve ter sua cobrança
      try {
        var _conta = (typeof getContaReceberByNota === "function") ? getContaReceberByNota(nf.id) : null;
        if (!_conta && typeof buildReceivableFromInvoice === "function") {
          _conta = buildReceivableFromInvoice(nf);
          contasReceber.push(_conta);
          if (typeof setIntegrationState === "function") setIntegrationState(_conta, "bancaria", { status: "autorizada_pronta_para_cobranca", lastAction: "autocura_status_por_prova" });
          if (typeof saveContasReceber === "function") saveContasReceber();
        }
      } catch (_) {}
      curadas++;
      if (typeof gdpLog === "function") gdpLog("[autocura] NF " + (nf.numero || nf.id) + " status→autorizada (tem chave+protocolo, status estava '" + (nf.status) + "').");
    });
    if (curadas && typeof gdpWarn === "function") gdpWarn("[autocura] " + curadas + " NF(s) com prova sincronizadas p/ autorizada.");
  } catch (e) { if (typeof gdpWarn === "function") gdpWarn("[autocura] falha em autoCurarStatusPorProva", e && e.message); }
  return { curadas: curadas };
}
if (typeof window !== "undefined") window.autoCurarStatusPorProva = autoCurarStatusPorProva;

// FRENTE 2 (recuperação MANUAL sob demanda) — botão/Console p/ recuperar UMA NF presa.
// As notas que JÁ travaram (autorizou na SEFAZ → voltou a 'pendente' apagando a chave) não são
// recuperadas pelo boot quando o throttle de 1x/hora da Distribuição DFe já esgotou com testes.
// Esta função FORÇA a recuperação de uma nota específica: primeiro tenta por chave (se houver chave
// de 44 díg, consulta a SEFAZ pelo protocolo), e se não tiver chave OU a consulta não autorizar,
// dispara a Distribuição DFe com { force:true } (ignora o throttle). Idempotente e conservadora:
// NUNCA marca autorizada sem prova SEFAZ real (chave+protocolo via cStat 100/150 + cSitNFe=1).
async function recuperarNotaManual(notaId) {
  try {
    var nf = (notasFiscais || []).find(function (n) { return n && n.id === notaId; });
    if (!nf) {
      if (typeof showToast === "function") showToast("Nota não encontrada para recuperação.", 5000);
      return;
    }
    // Idempotente: se já está autorizada (status OU prova SEFAZ), nada a fazer.
    if (nf.status === "autorizada" || (typeof temProvaAutorizacao === "function" && temProvaAutorizacao(nf))) {
      if (typeof showToast === "function") showToast("Nota " + (nf.numero || nf.id) + " já está autorizada.", 4000);
      return;
    }
    if (typeof showToast === "function") showToast("Recuperando NF " + (nf.numero || nf.id) + " na SEFAZ...", 5000);

    // 1) PRIMEIRO tenta por chave, SE houver chave de 44 dígitos (consulta o protocolo na SEFAZ).
    var chave = String((nf.sefaz && nf.sefaz.chaveAcesso) || nf.chaveAcesso || "").replace(/\D/g, "");
    if (chave.length === 44) {
      try {
        var resp = await fetch("/api/gdp-integrations", {
        ...
        var cStat = String(p.cStat || ""); var prot = p.nProt || p.prot || "";
        if ((cStat === "100" || cStat === "150") && String(prot).length > 0) {
          // PROVA SEFAZ real → marca autorizada (chave+protocolo+status).
          nf.sefaz = nf.sefaz || {};
          nf.sefaz.chaveAcesso = chave; nf.sefaz.protocolo = String(prot); nf.sefaz.status = "autorizada";
          nf.status = "autorizada";
          if (typeof setIntegrationState === "function") {
            setIntegrationState(nf, "sefaz", { status: "autorizada", lastAction: "recovery_manual_consulta", accessKey: chave, protocol: String(prot), cStat: cStat });
          }
          // cria cobrança faltante de forma IDEMPOTENTE (1 título por nota).
          var _conta = (typeof getContaReceberByNota === "function") ? getContaReceberByNota(nf.id) : null;
          if (!_conta && typeof buildReceivableFromInvoice === "function") {
            try {
              _conta = buildReceivableFromInvoice(nf);
              contasReceber.push(_conta);
              if (typeof setIntegrationState === "function") {
                setIntegrationState(_conta, "bancaria", { status: "autorizada_pronta_para_cobranca", lastAction: "recovery_manual_consulta" });
              }
              if (typeof saveContasReceber === "function") saveContasReceber();
            } catch (eCr) { gdpWarn("[recovery-manual] falha ao criar cobrança faltante", eCr && eCr.message); }
          }
          try { saveNotasFiscais(nf.id); } catch (_) {}
          try { if (window.gdpApi && window.gdpApi.notas_fiscais) window.gdpApi.notas_fiscais.save(nf); } catch (_) {}
          if (typeof renderAll === "function") renderAll();
          if (typeof showToast === "function") showToast("NF " + (nf.numero || nf.id) + " recuperada! (chave+protocolo confirmados na SEFAZ)", 6000);
          return;
        }
      } catch (_) { /* consulta falhou → segue para a Distribuição DFe */ }
    }

    // 2) Sem chave OU a consulta não autorizou: FORÇA a Distribuição DFe (ignora o throttle de 1x/hora).
    if (typeof _recuperarViaDistribuicaoDFe === "function") {
      try { await _recuperarViaDistribuicaoDFe({ force: true }); } catch (_) {}
    }
    if (typeof renderAll === "function") renderAll();
    // re-busca a nota (o estado pode ter mudado dentro da DFe).
    var nfPos = (notasFiscais || []).find(function (n) { return n && n.id === notaId; }) || nf;
    if (nfPos.status === "autorizada" || (typeof temProvaAutorizacao === "function" && temProvaAutorizacao(nfPos))) {
      if (typeof showToast === "function") showToast("NF " + (nfPos.numero || nfPos.id) + " recuperada!", 6000);
    } else {
      if (typeof showToast === "function") showToast("Não foi possível recuperar agora — a SEFAZ pode estar limitando consultas (tente em alguns minutos) ou a nota não consta como autorizada.", 8000);
    }
  } catch (e) {
    gdpWarn("[recovery-manual] falha em recuperarNotaManual", e && e.message);
    if (typeof showToast === "function") showToast("Erro ao recuperar a nota. Tente novamente em alguns minutos.", 6000);
  }
}
if (typeof window !== "undefined") window.recuperarNotaManual = recuperarNotaManual;
