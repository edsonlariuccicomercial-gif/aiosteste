...
function switchFinanceiroTab(subTab) {
  ...
  if (subTab === "caixa") { if (typeof renderCaixa === "function") renderCaixa(); _showPageFooter("caixa-page-footer"); }
  // Story 4.52: render functions now call footer updates internally with filtered data
...
function abrirDetalheCr(contaId) {
  ...
  body.innerHTML = `
    ...
    <div style="grid-column:1/-1;border:1px solid var(--bdr);border-radius:8px;padding:.8rem 1rem;margin-top:.4rem;background:rgba(255,255,255,.02)">
      ...
      <div style="font-size:.82rem;color:var(--mut);margin-bottom:.3rem">Boleto PDF: ${(function(){ var _cid = conta.cobranca?.providerChargeId || conta.integracoes?.bancaria?.providerChargeId || ''; if (_cid) { return '<a href="/api/bank-charge?action=bank-charge-pdf&providerChargeId=' + encodeURIComponent(_cid) + '" target="_blank" rel="noreferrer" style="color:var(--blue,#3b82f6);font-weight:700">abrir boleto</a>'; } return conta.cobranca?.bankSlipUrl ? '<a href="' + esc(conta.cobranca.bankSlipUrl) + '" target="_blank" rel="noreferrer" style="color:var(--blue,#3b82f6);font-weight:700">abrir boleto</a>' : '<span style="color:var(--yellow,#eab308)">em processamento — clique em Sincronizar</span>'; })()}</div>
...
(async function initGDP() {
  ...
  setTimeout(function() {
    ...
    // Patch lookup to use memoization during sanitize
    ...
    if (_origGetClientesVinculados) {
      ...
      getClientesVinculadosAoContrato = function(cid) {
        if (window._fastClienteCache[cid] !== undefined) return window._fastClienteCache[cid];
        var r = origFn(cid); window._fastClienteCache[cid] = r; return r;
      };
    }
