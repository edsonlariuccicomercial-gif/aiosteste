...
function novoPedidoManual() {
  ...
  // Fullpage mode: render in detalhe-page, hide listagem
...
function recalcSubtotalDetalhe(pedidoId, idx) {
  const p = pedidos.find(x => x.id === pedidoId);
  if (!p || !p.itens[idx]) return;
  // Story 14.7: Read qty from input (now editable)
  const qtdInput = document.getElementById("fiscal-item-qtd-" + pedidoId + "-" + idx);
  const precoInput = document.getElementById("fiscal-item-preco-" + pedidoId + "-" + idx);
  if (qtdInput) p.itens[idx].qtd = parseFloat(qtdInput.value) || 0;
  if (precoInput) p.itens[idx].precoUnitario = Math.round((parseFloat(precoInput.value) || 0) * 100) / 100;
  const sub = Math.round((p.itens[idx].qtd || 0) * (p.itens[idx].precoUnitario || 0) * 100) / 100;
...
function verPedidoDetalhe(pedidoId, isClone) {
  ...
  html += '<div style="margin-top:1.2rem;padding-bottom:1rem">';
...
function salvarPedidoCompleto(pedidoId) {
  ...
  (p.itens || []).forEach((item, idx) => {
    ...
    if (descEl) item.descricao = descEl.value.trim() || item.descricao;
...
function getCaixaResumo() {
  // Story 17.3: FONTE ÚNICA. O caixa lê SEMPRE de loadConciliacao() (espelho do Supabase,
  // tabela conciliacoes, scoped por empresa_id). O fallback legado caixaExtratoMovimentos
  // foi removido: ele era uma fonte em memória/legada que, num navegador com cache antigo,
  // divergia do banco (saldo mostrava só inicial enquanto a lista tinha lançamentos órfãos).
  // Render e cálculo agora usam a MESMA fonte → "visível = contabilizado".
  const concItems = typeof loadConciliacao === 'function' ? loadConciliacao() : [];
  const items = concItems;
  ...
  if (overflowTabs.length > 0) {
    const overflowItems = overflowTabs.map(tab => {
      ...
      return `<button onclick="setPedidoStatusTab('${tab.key}');closePedidoStatusDropdown()" style="display:flex;align-items:center;gap:8px;padding:8px 16px;background:transparent;border:none;cursor:pointer;width:100%;text-align:left;opacity:${active ? '1' : '.7'}">
        <span style="display:inline-flex;flex-shrink:0;width:8px;height:8px;border-radius:50%;background:${cor}"></span>
        <span style="font-size:.82rem;color:var(--txt);flex:1;font-weight:${active ? '700' : '400'}">${tab.label}</span>
