...
| 2026-06-19 | @devops (Gage) + @qa (Quinn) | **Fechamento do épico.** Re-gate por evidência de código + produção: 20.14 (gate PASS criado — NF-e desacoplada de _isAutorizado, AC10 coerência de status confirmada, gdp-notas-fiscais v19 em produção), 20.16 (CONCERNS→PASS — Causa A `[̀-ͯ]` confirmada por codepoint), 20.17 (CONCERNS→PASS — fix do ramo não-dirty confirmado + realtime verificado ao vivo via Playwright; AC4 destrutivo não executado por decisão do stakeholder). Status 20.14/20.15/20.16/20.17/20.18/20.19 → Done. Verificado: branch local == GitHub == produção. Remote git ajustado SSH→HTTPS. **Epic-20 concluído** (exceto testes destrutivos SEFAZ/2-navegadores, validáveis no uso normal — não bloqueantes). |
