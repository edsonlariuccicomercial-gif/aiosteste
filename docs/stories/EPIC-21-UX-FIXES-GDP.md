...
| 2026-06-22 | @devops (Gage) | PR #39 + merge em master (75f97c8); deploy prod `--force` (build cache descartado). Validação em produção: versões v=35/v=30/v=39 servidas; conteúdo confirmado — `pedido.status="faturado"` 2× (gerar+homologação), `_selectedPedidoIds.clear()` em aplicarSituacaoBulk, `.delete` nos baixa-handlers CP/CR. 21.15 + 21.7 → **Done** |
